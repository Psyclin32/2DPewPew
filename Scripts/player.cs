...
public partial class player : CharacterBody2D
{
	...
	{	
		//Declare local nodes
		var gun_Barrel = GetNode<Marker2D>("GunBarrelPos");
		...
		if (Input.IsActionJustPressed("Fire Weapon") & ready_fire)
		{
			...
			fire_Weapon(gun_Barrel.Position);
			ready_fire = false;
	...
	{
		 RigidBody2D projectile = (RigidBody2D) _bulletScene.Instantiate();
		 projectile.Visible = false;
		 projectile.Position = pos;
		 projectile.LookAt(GetGlobalMousePosition());
